            ...
            {autres.length > 0 ? (
              <aside className="flex min-h-0 flex-col">
                <div className="mb-3 flex items-baseline justify-between border-b border-line pb-2">
                  <span className="text-[11px] font-bold uppercase tracking-[0.1em] text-ink-3">Historique vidéo</span>
                  <span className="text-[11px] text-ink-3">{autres.length}</span>
                </div>
                {/* Liste défilante : TOUTES les autres vidéos sont accessibles,
                    plus de limite à 4. Hauteur bornée au lecteur en grand écran. */}
                <div className="flex flex-col overflow-y-auto pr-1 lg:max-h-[520px] [scrollbar-width:thin]">
                  {autres.map((v) => {
